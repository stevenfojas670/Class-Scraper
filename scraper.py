...
class Browser:
    ...
    def traverse_page(self):
        try:
            ...
            modules_container = self.browser.find_element(
                by=By.CLASS_NAME, value="ig-list"
            )
            module_sections = modules_container.find_elements(
                By.XPATH, ".//div[starts-with(@aria-label, 'Module')]"
            )
            ...
            for div in module_sections:
                ...
                for item in module_items:
                    try:
                        link_type_element = item.find_element(
                            by=By.CLASS_NAME, value="type_icon"
                        )
                        ...
                        if link_title == "External Tool":
                            link = item.find_element(
                                by=By.XPATH,
                                value=".//a[@class='ig-title title item_link']",
                            )
                            ...
                            self.get_request_information(
                                "https://emergingtalent.contentcontroller.com/vault",
                                module_name,
                                title,
                            )
                            self.wait_random_time()
    ...
    def download(self):
        ...
        try:
            ...
            # Save to a file
            with open(
                f"{cwd}\\{self.directory}\\{directory}\\{name}{file_extension}", "wb"
            ) as file:
                for chunk in response.iter_content(chunk_size=8192):
                    file.write(chunk)

            print(
                f"File downloaded successfully as '{cwd}\\{self.directory}\\{directory}\\{name}\\{file_extension}'"
            )
...
if __name__ == "__main__":
    ...
    # foundations = threading.Thread(
    #     target=run_multiple_drivers,
    #     args=(
    #         "place_name_of_course_here",
    #         "place_link_to_course_here",
    #         False,
    #     ),
    # )
    # foundations.start()
